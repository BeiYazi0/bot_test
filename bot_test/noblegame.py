...
DUEL_DB_PATH = './mydb/pcr_duel.db'
SCORE_DB_PATH = './mydb/pcr_running_counter.db'
...
#@sv.on_fullmatch(['贵族决斗帮助','贵族帮助','贵族指令'])贵族指令
def duel_help():
    msg='''
╔                                       ╗    
        贵族决斗相关指令
...
#@sv.on_prefix(['加载dlc','加载DLC','开启dlc','开启DLC'])贵族dlc开启
#gid频道加子频道
def add_dlc(gid:str, dlcname:str):
    if dlcname=='':
        return '请输入开启dlc的名字。'
    ...
    save_dlc_switch()
    return f'开启dlc {dlcname}  成功!'
        
    

#@sv.on_prefix(['卸载dlc','卸载DLC','关闭dlc','关闭DLC'])贵族dlc关闭
def delete_dlc(gid:str, dlcname:str):
    if dlcname=='':
        return '请输入关闭dlc的名字。'
    if dlcname not in dlcdict.keys():
        return 'DLC名填写错误'     
    ...
    save_dlc_switch()
    return f'关闭dlc {dlcname}  成功!'
    


#@sv.on_fullmatch(['dlc列表','DLC列表','dlc介绍','DLC介绍'])贵族dlc列表
def intro_dlc():
    msg = '可用DLC列表：\n\n'
    i=1
...
#@sv.on_fullmatch(['dlc帮助','DLC帮助','dlc指令','DLC指令'])贵族dlc指令
def help_dlc():
    msg = '''
╔                                 ╗
         DLC帮助
